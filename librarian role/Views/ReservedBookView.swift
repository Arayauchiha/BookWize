...
//    }
    ...
    func fetchReservations() async {
        do {
            let response: [ReservationRecord] = try await supabase
    ...
    func issueReservedBook(reservation: ReservationRecord) async {
        ...
        do {
            ...
            _ = try supabase.rpc("begin_transaction")
            ...
            // Commit the transaction
            _ = try supabase.rpc("commit_transaction")
            ...
            // Rollback the transaction if it was started
            _ = try? supabase.rpc("rollback_transaction")
